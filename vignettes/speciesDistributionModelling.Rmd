...
standardSDM <- bru(
  ~ temp(findCovar(.data., envCovars, "temp"), model = "linear") + prec(findCovar(.data., envCovars, "prec"), model = "linear") +
      tempSq(findCovar(.data., envCovars, "temp")^2, model = "linear") + precSq(findCovar(.data., envCovars, "prec")^2, model = "linear") + Intercept(1),
  like(
    formula = geometry ~ .,
    family = "cp",
    data = occData,
...
standardSDMPred <- predict(standardSDM, fm_pixels(effMesh, mask = norwayVector), ~ exp(temp + prec + tempSq + precSq + Intercept))
...
```
```{r}
standardSDMPred_resp <- predict(standardSDM, , ~ exp(temp + prec + tempSq + precSq + Intercept))
```

```{r spatialSDMImplementation}
# Construct the latent model components
maternField <- inla.spde2.pcmatern(effMesh, prior.sigma = c(0.1, 0.01), prior.range = c(0.1, 0.01))
# Fit the model
spatialSDM <- bru(
  ~ temp(findCovar(.data., envCovars, "temp"), model = "linear") + prec(findCovar(.data., envCovars, "prec"), model = "linear") +
...
spatialSDMPred_full <- predict(spatialSDM, fm_pixels(effMesh, mask = norwayVector), ~ exp(spatSmooth + temp + prec + tempSq + precSq + Intercept))
